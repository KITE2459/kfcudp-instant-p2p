 ...
 * 추방과 달리 재입장을 막지 않는 1회성 강퇴다(ExpelManager 클래스 주석 참고).
 * <b>강퇴·추방을 별도 화면으로 다시 떼어내지 말 것</b> — 예전엔 그랬는데, 여기서 차단(❌)을
 * 누르면 어차피 추방 요청도 같이 나가서(아래 handleClick 참고) 화면 둘이 같은 일을 반씩 나눠
 * 하는 꼴이었고, 그래서 같은 동작인데 화면마다 아이콘·색이 어긋나는 버그도 났다.
 * <p>
 * 닉네임 왼쪽 얼굴 아이콘은 접속 중이면 탭 목록 텍스처를 바로, 아니면 세션 서버에서
